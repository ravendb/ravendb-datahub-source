...
class RavenDBConfig(EnvConfigMixin):
    ...
    certificate_file_path: str = Field(
        default=None,
        description="Path to client certificate file"
    )

    trust_store_file_path: str = Field(
        default=None,
        description="Path to trust store file"
    )

    # add more config values like username/password, etc.
...
class RavenDBSource(Source):
    ...
    def _create_document_store(self, config: RavenDBConfig, database = None) -> DocumentStore:
        """
        Create a RavenDB DocumentStore
        Parameters
        ----------
            config:
                the RavenDB configuration
        ...
        logging.debug(f"Initialize store: {self.config.connect_uri} for database: {database}")

        store = DocumentStore(config.connect_uri, database)
        if config.certificate_file_path is not None:
            store.certificate_pem_path = config.certificate_file_path

        if config.trust_store_file_path is not None:
            store.trust_store_path = config.trust_store_file_path
    ...
    def get_database_names_pagination(self, start=0, page_size=10):
        ...
        """
        result = []
        store = self._create_document_store(self.config)
        while True:
            operation = GetDatabaseNamesOperation(start=start, page_size=page_size)
            database_names = store.maintenance.server.send(operation=operation)
    ...
    def get_workunits(self) -> Iterable[MetadataWorkUnit]:
        ...
        for database in sorted(self.database_names):
            ...
            db_store = self._create_document_store(self.config, database)
